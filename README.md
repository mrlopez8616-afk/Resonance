...
**Resonance 2.0** (operator floor + agentic sleeve fill log) is a separate Next.js app in [`resonance-2/`](resonance-2/). Point Vercel project `resonance3` at Root Directory `resonance-2`. This Phase Zero dashboard is unchanged.
...
- **Reports** — Digital binder for briefs/reports the OS files. Left-rail toolbox (not a node). Date folders on `/reports` and `/archive/YYYY-MM-DD` (America/Chicago day key) → file on `/reports/[id]`. Schema: `id`, `title`, `kind` (`brief | rh-ops | build | other`), `createdAt`, `body` or blob pointer, `fingerprint` (SHA-256 of the filed record), Hedera Testnet attest (`hashgraph_attested` + message id / HashScan link). Shared store: `GET/POST /api/reports` (same Blob family, `resonance/reports.json`). Hub Bearer-files with `dayKey`; same day + title merges. Attest reuses `POST /api/attest`. File-from-Decision on the Decision page. Contract: [`docs/reports-autofile.md`](docs/reports-autofile.md). Printer is later. Health reports `reportsSync.itemCount` only.
