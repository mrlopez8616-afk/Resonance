...
| Piece | Path | Use |
| --- | --- | --- |
| Live face UI | `src/components/live-node-face.tsx` | Ticker and `$price` share the same large size; `{units} tokens`, `~$usd live`, and sleeve rows are the next size down and stay inside the tile. Polls `/api/spot-price?ticker=` and `/api/sleeves?ticker=` |
...
- Coinbase: face row is `Coinbase 33.7` only. The old “(buy print / staked)” label is omitted for UI cleanliness.
- Buy-print rule (internal, not shown on the face): Coinbase Advanced Trade (2026-09-19) `balance` SUI available=0 hold=0 (staking not exposed on this API key). Quantity is the sum of SUI-USD **FILLED buys** only: 16.9 + 16.8 = 33.7. Sells on this book: none. Do **not** invent a separate stake size beyond 33.7.
