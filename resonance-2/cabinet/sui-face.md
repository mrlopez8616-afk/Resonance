...
| Piece | Path | Use |
| --- | --- | --- |
| Live face UI | `src/components/live-node-face.tsx` | Ticker and `$price` share the same large size; `{units} tokens`, `~$usd live`, and sleeve rows are bigger and fill the square. Polls `/api/spot-price?ticker=` |
