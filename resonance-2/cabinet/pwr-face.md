...
PWR is the third live node and the first **equity** face. Do not copy-paste a fourth face. Reuse the shared pieces. Do not pretend CoinGecko or Binance has PWR.
...
| Face shape | `src/lib/live-face.ts` → `assembleLiveFace(ticker, sleeves, quote)` | Positions from config. Only `quote.usd` is live. Face qty is `formatSleeveQuantity` — values under 1 keep six decimals so `0.001578` does not round to `0.002` |
| Sleeve type | `src/data/sleeves.ts` → `NodeSleeve` | Quantity stays a string. Face rows are label + quantity only |
| Sleeve print | `src/data/pwr-sleeves.ts` | RH Agentic `0.001578` only |
...
| Ingest | [`fill-ingest.md`](./fill-ingest.md) | PWR `rh-agentic` is now a live book. `rh-main` / `coinbase` are **not** on this face (400). Flare vault untouched |
...
`src/data/pwr-sleeves.ts` — founder lock 2026-09-19. Do not invent lots.

- RH Agentic: `0.001578` **shares**
- No RH Main line
- No Coinbase line
- No Flare vault
- No Xaman / gas wallet address

Headline: `0.001578 shares`, then `0.001578 × live PWR-USD` as `~$… live`. Ticker and live price stay the large pair; shares / live USD / the one sleeve row step down and stay inside the square (face-fit #31).

Durable overrides land via fill ingest on `rh-agentic` only. Seed `0.001578` remains the fallback until a hub POST applies.
